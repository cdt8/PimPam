...
static ans_t __imp_hoof4_2(sysname_t tasklet_id, node_t root, node_t second_root) {
    ...
    return (common_size * (common_size - 1)) >> 1;
}

static ans_t __imp_hoof4(sysname_t tasklet_id, node_t root) {
    edge_ptr root_begin = row_ptr[root];  // intended DMA
    ...
    for (edge_ptr i = root_begin; i < root_end; i++) {
        ...
        ans += __imp_hoof4_2(tasklet_id, root, second_root);
    }
    return ans;
}

extern void hoof4(sysname_t tasklet_id) {
    ...
    while (i < root_num) {
        ...
        for (edge_ptr j = root_begin + tasklet_id; j < root_end; j += NR_TASKLETS) {
            ...
            if (second_root >= root) break;
            partial_ans[tasklet_id] += __imp_hoof4_2(tasklet_id, root, second_root);
    ...
    for (i += tasklet_id; i < root_num; i += NR_TASKLETS) {
        ...
        timer_start(&cycles[tasklet_id]);
#endif
        ans[i] = __imp_hoof4(tasklet_id, root);  // intended DMA
#ifdef PERF
